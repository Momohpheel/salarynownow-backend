...
class MarketplaceEnquiryController extends Controller
{
    public function store(Request $request)
    {
        ...
        $enquiry = MarketplaceEnquiry::create([
            ...
            'company_name' => $request->company_name ?? ($user->company_name ?? null),
        ...
        try {
            if ($partnerId) {
                Notification::notify((int)$partnerId, [
                    ...
                    'body' => ($user->company_name ?? $user->name ?? 'An employer') . ' sent an enquiry' . ($request->offer_name ? " about {$request->offer_name}" : '') . '.',
                    'icon' => 'message-square',
    ...
    public function index(Request $request)
    {
        $user = $request->user();
        $employerId = $user->getEmployerId();
        $rows = MarketplaceEnquiry::where(function ($q) use ($user, $employerId) {
            $q->where(function ($sub) use ($user) {
                $sub->where('submitter_type', User::class)
                    ->where('submitter_id', $user->id);
            })->orWhere(function ($sub) use ($employerId) {
                $sub->where('submitter_type', User::class)
                    ->where('submitter_id', $employerId);
            });
        })
