...
class OwnerGroupController extends Controller
{
    public function index(Request $request)
    {
        ...
        $allEmployerChildren = $admin->children()
            ->where('type', User::TYPE_EMPLOYEE)
            ->get();

        if ($allEmployerChildren->isEmpty()) {
            return $this->sendResponse([], 'Owner groups retrieved successfully');
        }

        $employerIds = $allEmployerChildren->pluck('id')->toArray();

        $ownerUserIds = [];
        foreach ($allEmployerChildren as $emp) {
        ...
        $activeAdvanceCounts = SalaryAdvance::whereIn('user_id', $allGroupBusinessIds)
            ->orWhere(function ($q) use ($staffUserIds) {
                if (!empty($staffUserIds)) {
                    $q->whereIn('staff_id', $staffUserIds);
                } else {
                    $q->whereRaw('1 = 0');
                }
            })
        ...
        foreach ($groupMapping as $ownerId => $businessIds) {
            ...
            foreach ($businessIds as $bid) {
                $biz = $allEmployerChildren->firstWhere('id', $bid);
                if (!$biz) {
                    $biz = User::find($bid);
                }
                if (!$biz) {
